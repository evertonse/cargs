...
#<<=========================================================================================================
  def __init__(self,toml_dict, abs_path=False) -> None:
    ...
    create_path = lambda f : f'{Path(f)}'
    self.workingdir = Path(os.getcwd()).resolve() if not hasattr(self,'workingdir') else Path(self.workingdir) 
    if abs_path == True:
        create_path = lambda f :f'"{Path(f).resolve()}"'
        self.use_abs = True
